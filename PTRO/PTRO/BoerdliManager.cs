...
namespace PTRO
{
    class BoerdliManager
    {
        private bool boerdliConnected = false;
        private Timer boerdliTimer = new Timer();
        ...
        public bool IsAvaiable()
        {
            ...
            try
            {
                ...
                if (h == 0)
                {
                    SimpleLogger.log("Boerdli connected");
                    boerdliConnected = true;
                    return true;
                }
                else
                {
                    SimpleLogger.log("Boerdli not connected");
                }

            }
            catch (Exception e)
            {
                SimpleLogger.log(e.ToString());
            }
